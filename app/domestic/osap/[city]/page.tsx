...
import { OSAP_CITIES, osapCityBySlug } from "@/lib/data/domestic-cities";
import { CtaBlock } from "@/components/sections/cta-block";
import { TrustBar } from "@/components/sections/trust-bar";

export function generateStaticParams() {
  return OSAP_CITIES.map((c) => ({ city: c.slug }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const data = osapCityBySlug[city];
  if (!data) return {};

  const title = `Free OSAP Help in ${data.name}, Ontario | Study Master Canada`;
  const description = `Free OSAP application help for ${data.name} students. 2,000+ Ontario students coached. Applications, reviews, and appeals — 100% free. WhatsApp us today.`;

  return {
    title,
    description,
    keywords: `OSAP ${data.name}, OSAP help ${data.name}, ${data.name} OSAP application, free OSAP help ${data.name}, Ontario student aid ${data.name}, OSAP ${data.region}`,
    openGraph: {
      title,
      description,
      type: "website",
    ...
    alternates: { canonical: `${siteUrl}/domestic/osap/${data.slug}` },
  };
}

export default async function OsapCityPage({ params }: PageProps) {
  const { city } = await params;
  const data = osapCityBySlug[city];
  if (!data) notFound();
  ...
  const serviceJsonLd = {
    ...
    "@type": "Service",
    name: `Free OSAP Help in ${data.name}`,
    serviceType: "Student Financial Aid Consulting",
    description: `Free OSAP application, review, and appeal support for ${data.name} post-secondary students.`,
    areaServed: {
      "@type": "City",
      name: data.name,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "OSAP Help", item: `${siteUrl}/domestic/osap` },
      { "@type": "ListItem", position: 4, name: `OSAP Help in ${data.name}`, item: `${siteUrl}/domestic/osap/${data.slug}` },
    ],
  };
    ...
    <main>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          <nav className="text-xs text-white/80" aria-label="Breadcrumb">
            <ol className="flex flex-wrap items-center gap-2">
              ...
              <li className="opacity-50">/</li>
              <li><Link className="text-white hover:underline" href="/domestic/osap">OSAP Help</Link></li>
              <li className="opacity-50">/</li>
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_minmax(260px,300px)] lg:items-start lg:gap-10">
            ...
            <aside className="rounded-2xl border border-white/15 bg-white/5 p-5 backdrop-blur">
              <p className="text-xs font-bold uppercase tracking-widest text-sm-orange-light">OSAP at a glance</p>
              <dl className="mt-5 space-y-4">
                {[
                  { k: "4–6 wks", v: "Typical processing" },
                  { k: "25% max", v: "Grant portion (Fall 2026+)" },
                  { k: "$0", v: "Our counselling fee" },
                  { k: "2,000+", v: "Ontario files coached" },
                ].map((row) => (
      ...
      <section className="bg-sm-off-white py-12 md:py-16">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          <div className="border-l-4 border-sm-orange pl-3">
            <p className="text-xs font-bold uppercase tracking-widest text-sm-teal-dark">OSAP IN {data.name.toUpperCase()}</p>
          </div>
          <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-sm-gray-900 md:text-4xl">
            How OSAP works for {data.name} students
          </h2>
      ...
      <section className="bg-white py-12 md:py-14">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          ...
          <p className="mt-2 text-sm text-sm-gray-600">All institutions below are OSAP-designated (subject to individual program verification).</p>
      ...
      <section className="bg-sm-off-white py-12 md:py-14">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          ...
          <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-sm-gray-900 md:text-3xl">
            What we do for {data.name} OSAP students
          </h2>
            ...
            {[
              {
                ...
                body: `We confirm your Ontario residency, dependency status, and that your ${data.name} institution is OSAP-designated — before you apply.`,
              },
              {
                Icon: ClipboardList,
                title: "Document prep",
                body: "SIN, income proof, enrolment letters, MSFAA — we build your checklist and review every upload before you submit.",
              },
              {
                Icon: DollarSign,
                title: "Maximize grants",
                body: "We structure your file to surface every eligible grant dollar — especially critical as Fall 2026 shifts more OSAP funding to loans.",
              },
              {
                Icon: MessageCircle,
                title: "Portal walkthroughs",
                body: "Ontario's OSAP portal has confusing fields. We walk through it with you — over WhatsApp, video call, or in our Brampton office.",
              },
              {
                Icon: CheckCircle2,
                title: "Reviews & appeals",
                body: "Assessment look wrong? Income changed? We write and submit formal OSAP reviews and appeals — at no cost to you.",
              },
              {
                Icon: MapPin,
                title: "Local knowledge",
                body: `We know ${data.name}'s post-secondary landscape — which programs are designated, which have known issues, and what to watch for.`,
              },
            ].map(({ Icon, title, body }) => (
      ...
      <section className="bg-sm-teal py-6">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-4 px-6 text-center md:flex-row md:justify-between md:px-10 md:text-left">
          <div>
            <p className="font-bold text-white">Ready to start your OSAP application in {data.name}?</p>
            <p className="mt-1 text-sm text-white/80">Free for all Ontario students. WhatsApp, call, or walk in.</p>
          </div>
          <a
            href={SITE.whatsapp}
            target="_blank"
      ...
      <section id="faqs" className="scroll-mt-28 bg-white py-12 md:py-16">
        <div className="mx-auto max-w-4xl px-6 md:px-10">
          ...
          <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-sm-gray-900 md:text-4xl">
            OSAP questions from {data.name} students
          </h2>
      ...
      <section className="bg-sm-off-white py-8 border-t border-sm-gray-100">
        <div className="mx-auto max-w-7xl px-6 md:px-10 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-sm-gray-700">
            <strong>Want the full OSAP guide?</strong>{" "}
            <Link href="/domestic/osap" className="font-semibold text-sm-teal-dark underline hover:no-underline">
              Read our complete OSAP 2026 overview →
            </Link>
          </p>
          <p className="text-sm text-sm-gray-700">
            <strong>In British Columbia?</strong>{" "}
      ...
      <CtaBlock
        ...
        primaryLabel={`WhatsApp for Free OSAP Help in ${data.name}`}
        heading={`Questions about OSAP in ${data.name}?`}
        body="Free for all Ontario students. Applications, reviews, and appeals — handled with care."
      />
    </main>
  );
}
